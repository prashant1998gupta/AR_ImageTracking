...
public static class RishabhSceneBuilder
{
    ...
    //
    // The line below is REWRITTEN IN PLACE by
    // Tools ▸ Rishabh ▸ Read Layout From Scene — keep it on one line, keep the
    // marker directly above it, and do not add anything between the two.
    // ─── CONTENTSCALE AUTO-LINE — regenerated by Tools ▸ Rishabh ▸ Read Layout From Scene
    ...
    // (Canvas ▸ ScanArea ▸ CardImage + ScanToStart).
    //   false = KEEP it — the tracker's OnImageFound hides it, OnImageLost shows it
    //           again, so the visitor always knows what to point the camera at.
    //   true  = delete it entirely and clear those two listeners.
    // The empty screen Canvas is only auto-removed when this leaves it childless.
    private static readonly bool RemoveScanArea          = false;
    // Repoints the prompt's CardImage at THIS card's art (instead of the demo card)
    // and reshapes it to the portrait aspect. Ignored when RemoveScanArea is true.
    ...
    //
    // ROUND-TRIP: everything between the two AUTO markers below is REGENERATED by
    // Tools ▸ Rishabh ▸ Read Layout From Scene (write into builder CONFIG). That
    // command reads the arranged scene and rewrites ONLY dx / dy / width / aspect —
    // sprite, url, vCard and worldZ are carried through from the entries as they
    // stand, and the comments between entries travel with the entry beneath them.
    // Arrange the card by hand in Rishabh.unity, run that command, and the hand work
    // survives the next rebuild. Do not delete the markers.
    private static readonly List<Element> Layout = new List<Element>
    {
        // ─── LAYOUT AUTO-BEGIN — regenerated by Tools ▸ Rishabh ▸ Read Layout From Scene
        ...
        new Element { name = "linesBg",        sprite = "Lines.png",         dx =    0.09f, dy =   0.127f, width =  1.823f, aspect = 0.8686f, worldZ = LinesWorldZ },

        new Element { name = "rionickTop",     sprite = "Rionick.png",       dx =  0.0235f, dy =  0.9029f, width = 0.7073f, aspect = 0.2781f, url = "https://www.rionick.com" },
        new Element { name = "ghar360",        sprite = "ghar360 Icon.png",  dx = -0.7638f, dy =  0.4406f, width =  0.379f, aspect = 1.0094f, url = "https://rionick.com/ghar360" },
        new Element { name = "arrise",         sprite = "ARRISE Icon.png",   dx = -0.7638f, dy = -0.0209f, width = 0.3803f, aspect = 1.0189f, url = "https://ar.rionick.com/" },

        new Element { name = "name",           sprite = "Name.png",          dx =  0.9943f, dy =   0.414f, width =  0.914f, aspect =  0.508f },
        new Element { name = "letsConnect",    sprite = "lets connect.png",  dx =   0.949f, dy =  0.1166f, width =  0.532f, aspect = 0.1948f },

        new Element { name = "linkedin",       sprite = "Linkedin.png",      dx =  0.6883f, dy = -0.0943f, width = 0.2369f, aspect = 1.0177f, url = "https://www.linkedin.com/company/rionick-studios/" },
        new Element { name = "instagram",      sprite = "Instagram.png",     dx =  0.9569f, dy = -0.0943f, width =  0.235f, aspect = 1.0088f, url = "https://www.instagram.com/rionick.studios/" },
        new Element { name = "whatsapp",       sprite = "Whatsapp.png",      dx =  1.2139f, dy = -0.0943f, width = 0.2323f, aspect =      1f, url = "https://wa.me/918377935429" },

        new Element { name = "saveContact",    sprite = "Save Contact.png",  dx =  0.9662f, dy = -0.3672f, width = 0.7501f, aspect = 0.2919f, vCard = true },
        new Element { name = "rionickBottom",  sprite = "Rionick.png",       dx =       0f, dy = -0.9163f, width = 0.5545f, aspect = 0.2918f, url = "https://www.rionick.com" },
        // ─── LAYOUT AUTO-END
    };
